...
function pickUaFromTranslations(res, type){
  try{
    ...
    for (var i=0;i<tr.length;i++){
      ...
      if (!t) continue;
      if (String(t.iso_639_1 || '').toLowerCase() === 'uk') { ua = t; break; }
      if (String(t.iso_3166_1 || '').toUpperCase() === 'UA') { ua = t; /* не break, але ок */ }
    }
    if (!ua || !ua.data) return '';
  ...
  function applyTheme(theme) {
    ...
    var themeCss = {
      ...
      apple: 
        ...
        m + ' { background: rgba(30, 30, 30, 0.4) !important; border: 1px solid rgba(255, 255, 255, 0.2) !important; border-radius: 1em !important; -webkit-backdrop-filter: blur(25px) !important; backdrop-filter: blur(25px) !important; }',
