(function() {
    ...
    // Configuration
    var CACHE_TIME = 3 * 24 * 60 * 60 * 1000; // 3 days
    var CACHE_NAME = 'maxsm_rating_cache';
    var SETTINGS_COMPONENT_NAME = 'omdb_ratings_mod';
    ...
    Lampa.Lang.add({
        ...
        mdblist_api_key_label: {
            ru: 'MDBList API ключ',
            en: 'MDBList API Key',
            uk: 'MDBList API ключ'
        },
        icon_color_style_label: {
            ru: 'Стиль иконок',
            en: 'Icon Style',
            uk: 'Стиль іконок'
        },
    ...
    function applyStyles() {
        var icon_filter = Lampa.Storage.get(SETTINGS_COMPONENT_NAME + '_icon_style', 'color') === 'bw' ? 'grayscale(100%)' : 'none';
        ...
        <style id="maxsm_omdb_rating_mod">
            ...
            .loading-dots {
                ...
                align-items: center;
                gap: 0.4em; color: #fff;
                font-size: 1em; background: rgba(0,0,0,0.3);
                padding: 0.6em 1em; border-radius: 0.5em;
            }
            /* ... other loading styles from original ... */
        </style>`;
    ...
    function addSettings() {
        Lampa.SettingsApi.addComponent({
            ...
            name: Lampa.Lang.translate('settings_omdb_mod_title'),
            icon: '<svg ... > ... </svg>' // Placeholder for component icon
        });

        Lampa.SettingsApi.addParam({
            component: SETTINGS_COMPONENT_NAME,
            param: { name: 'omdb_api_key', type: 'input', "default": '12c9249c' },
            field: { name: Lampa.Lang.translate('omdb_api_key_label') }
        });

        Lampa.SettingsApi.addParam({
            component: SETTINGS_COMPONENT_NAME,
            param: { name: 'mdblist_api_key', type: 'input', "default": '' },
            field: { name: Lampa.Lang.translate('mdblist_api_key_label') }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                ...
                "default": 'color',
                values: {
                    'color': Lampa.Lang.translate('icon_style_color'),
                    'bw': Lampa.Lang.translate('icon_style_bw')
                }
            },
            field: { name: Lampa.Lang.translate('icon_color_style_label') },
    ...
    /**
     * Parse awards text from OMDB API response.
     * @param {string} awardsText - The 'Awards' string from OMDB.
     * @returns {object|null} - Parsed awards data.
     */
    ...
    /**
     * Main function to fetch and display ratings.
     * @param {object} card - The Lampa movie/tv card object.
     */
    function fetchAndDisplayRatings(card) {
        ...
        let omdb_api_key = Lampa.Storage.get(SETTINGS_COMPONENT_NAME + '_omdb_api_key', '12c9249c');
        let mdblist_api_key = Lampa.Storage.get(SETTINGS_COMPONENT_NAME + '_mdblist_api_key', '');

        // Create promises for both API calls
        const omdbPromise = imdb_id ? new Promise((resolve) => {
            let url = `https://www.omdbapi.com/?apikey=${omdb_api_key}&i=${imdb_id}`;
            Lampa.Request.get(url, (data) => resolve(data), () => resolve({}));
        }) : Promise.resolve({});

        const mdbListPromise = mdblist_api_key ? new Promise((resolve) => {
        ...
        Promise.all([omdbPromise, mdbListPromise]).then(([omdbData, mdbListData]) => {
            ...
            // Prioritize MDBList for ratings
            ...
            // Fallback to OMDB for ratings if MDBList is missing them
            ...
            // Always get TMDB rating from the card itself as a fallback
            if (!finalRatings.tmdb && card.vote_average) {
                finalRatings.tmdb = parseFloat(card.vote_average.toFixed(1));
            }
            
            // Now, render the ratings
            renderRatings(finalRatings, render);
        });
    }
    
    /**
     * Render the fetched ratings into the UI.
     * @param {object} ratings - The consolidated ratings object.
     * @param {jQuery} render - The jQuery object for the card's render area.
     */
    function renderRatings(ratings, render) {
        let rateLine = $('.full-start-new__rate-line', render);
        rateLine.empty(); // Clear loading indicators
        ...
        if (ratingsCount > 1 && totalWeight > 0) {
            ...
            rateLine.append(`
                <div class="rate-item rate--avg ${colorClass}">
                    ...
                    <div class="rate-item__value">${average}</div>
                </div>`);
        }
        ...
        if (ratings.oscars > 0) {
            rateLine.append(`
                <div class="rate-item rate--oscars">
                    <div class="rate-item__value">${ratings.oscars} Oscars</div>
                </div>`);
        }
        if (ratings.emmy > 0) {
            rateLine.append(`
        ...
        if (ratings.rt) {
            let rt_icon = ratings.rt >= 60 ? rotten_good_svg : `<img class="rate-item__icon" src="${rotten_bad_icon_url}">`;
            rateLine.append(`
