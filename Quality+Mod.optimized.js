...
(function() {
    ...
    // ===================== RUNTIME SPEED OPTIMIZATIONS (no logic change) =====================
    // In-memory session cache to avoid repeated localStorage hits on weak devices
    var LQE_SESSION_CACHE = Object.create(null);
    // Track processed list cards without touching DOM attributes (less reflow)
    var LQE_PROCESSED_CARDS = new WeakSet();
    // Single IntersectionObserver to process only visible/near-visible cards
    var LQE_IO = null;
    try {
        LQE_IO = new IntersectionObserver(function(entries){
            entries.forEach(function(entry){
                var el = entry.target;
                if (entry.isIntersecting && el && !LQE_PROCESSED_CARDS.has(el)) {
                    LQE_PROCESSED_CARDS.add(el);
                    // Process immediately (will do instant paint from cache if available)
                    try { updateCardListQuality(el); } catch(e) {}
                    // No need to keep observing once processed
                    LQE_IO.unobserve(el);
                }
            });
        }, { root: null, rootMargin: '300px 0px', threshold: 0.01 });
    } catch(e) {
        // If IO is unavailable, we'll fallback to immediate processing in observer
        LQE_IO = null;
    }
...
function simplifyQualityLabel(fullLabel, originalTitle) {
    ...
    function getQualityCache(key) {
        ...
        if (LQE_SESSION_CACHE[key]) return LQE_SESSION_CACHE[key];
    ...
    function saveQualityCache(key, data, cardId) {
        // Update in-memory session cache first for instant re-use
        LQE_SESSION_CACHE[key] = { quality_code: data.quality_code, full_label: data.full_label, timestamp: Date.now() };
	...
	if (bypassTranslation && LQE_CONFIG.USE_SIMPLE_QUALITY_LABELS) {
        ...
        if (existing) {
            ...
            if (!inner) { inner = document.createElement('div'); existing.appendChild(inner); }
            if (inner.textContent !== displayQuality) { inner.textContent = displayQuality; }
            if (!existing.classList.contains('show')) { requestAnimationFrame(function(){ existing.classList.add('show'); }); }
            return;
        }

        // Створюємо новий елемент (вперше)
    ...
    function updateCardListQuality(cardElement) {
        ...
        if (LQE_PROCESSED_CARDS.has(cardElement)) {
        ...
        LQE_PROCESSED_CARDS.add(cardElement); // Позначаємо як оброблену (без DOM мутації)
    ...
    function debouncedProcessNewCards(cards) {
        ...
        observerDebounceTimer = setTimeout(function() {
            var uniqueCards = cards.filter(function(card){ return card && card.isConnected; });
            if (!uniqueCards.length) return;

            // For each card: if IntersectionObserver available, observe; else process immediately (microtask)
            uniqueCards.forEach(function(card){
                try {
                    if (LQE_IO) {
                        LQE_IO.observe(card);
                    } else {
                        if (!LQE_PROCESSED_CARDS.has(card)) {
                            LQE_PROCESSED_CARDS.add(card);
                            // Immediate paint from cache then network inside function
                            Promise.resolve().then(function(){ updateCardListQuality(card); });
                        }
                    }
                } catch(e){}
            });
